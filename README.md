...
当前如果目标是验证 AI 主观选股是否有效，并愿意接受更高波动来增加交易样本，应明确使用模拟探索模式：

```text
使用 $stock-agent-pipeline 以 exploration_mode 模拟探索模式运行今天的 AI 主观选股模拟调仓，用于验证框架有效性。
```

如果未明确指定风险模式，项目仍按 `safe_mode` 安全模式运行。
