...
6. 使用 `references/style-contract.md` 中定义的启用风格，并传入当前状态、候选、市场信息和决策记忆上下文。
7. 判断 `buy`、`watch`、`reject`、`hold`、`sell_all`、`trim` 或 `no_action`。
8. 执行 `references/hard-rules.md` 中的非仓位类硬规则。
9. 如果最终结论为空仓、观察、持有或不动，跳过交易执行并继续生成报告。
10. 如果最终结论需要模拟买入或卖出，调用模拟仓事实源，随后再次查询事实源状态。
...
- 数据发现智能体：使用 `mx-xuangu`、`mx-data`、`mx-search` 和 iFind 形成候选与上下文证据。
...
- 模拟调仓或选股研究后，更新 `reports/memory/trading-memory.md`；该文件不得作为账户账本。
